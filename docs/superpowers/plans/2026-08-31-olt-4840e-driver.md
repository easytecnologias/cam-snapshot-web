...
def test_reboot_onu_4840e_never_sends_bare_reboot():
    """Garante que nenhum comando enviado pelo driver e a string 'reboot'
    isolada (sem o prefixo 'onu-') -- esse comando reinicia a OLT inteira."""
    def script(cmd, prompt):
        if cmd == "conf t":
            return "", "OLT_RADS(config)#"
        if cmd == "onu 0/4/6":
    ...
    orig_open_shell = _patch_open_shell(script, prompt="OLT_RADS(config)#")
    orig_login, orig_enable = _patch_login()
    try:
        mod.reboot_onu_4840e("100.64.10.5", "admin", "x", pon=4, onu=6)
        sent = mod._open_shell.__closure__  # nao usado -- commands vem do chan, ver abaixo
    finally:
        _unpatch(orig_open_shell, orig_login, orig_enable)
    # Reforco direto na fonte: nenhuma funcao deste modulo pode montar a
    # string 'reboot' sem prefixo -- checagem estatica simples do arquivo.
    import inspect
    source = inspect.getsource(mod)
    check(
        not re.search(r'(?<!onu-)(?<!["\'])\breboot\b(?!\w)', source.replace("onu-reboot", "").replace("reboot_onu_4840e", "").replace("def reboot", "")),
        "achou a palavra 'reboot' isolada no codigo fonte fora de onu-reboot/reboot_onu_4840e",
    )
...
Run: `python scripts/sightops_olt_4840e_add_onu_test.py`
Expected: `OK: ...`, sem falhas — em especial os dois testes de reboot
passando.
