...
def add_onu_4840e(
    ...
    try:
        ...
        for entry in ports:
            ...
            # NOTA (validado em equipamento real, OLT BARRA DE SAO MIGUEL):
            # a porta ethernet TAMBEM leva o prefixo de slot "0/", igual a
            # PON -- "interface ethernet 1" (sem o "0/") falha com "%
            # Invalid parameter". A primeira versao deste plano tinha esse
            # bug; so foi pego na validacao ao vivo porque os testes com
            # mock nunca validam a sintaxe real do comando.
        ...
        # NOTA (validado em equipamento real): "copy running-config
        # startup-config" TAMBEM pede confirmacao y/n nesta OLT ("Startup
        # config in flash will be updated, are you sure(y/n)? [n]"). Um
        # _cli() simples nunca responde a essa confirmacao -- a sessao
        # fecha, a OLT assume o default [n] (nao salva), e o driver
        # reportava "saved: True" mesmo sem ter salvo nada de verdade (o
        # texto de confirmacao nao bate com nenhum _FAILURE_MARKERS). Por
        # isso usa o mesmo padrao de confirmacao controlada do reboot --
        # ver _cli_confirm_save mais abaixo (compartilhado com
        # delete_onu_4840e).
...
def delete_onu_4840e(
    ...
    try:
        ...
        saved = False
        if ok:
            # ver a nota de "copy running-config startup-config" no
            # add_onu_4840e -- essa OLT pede confirmacao y/n pra salvar,
            # validado ao vivo (equipamento real).
            cmd = "copy running-config startup-config"
            saved = _cli_confirm_save(chan, timeout=max(timeout, 25.0))
            commands_run.append(cmd)
            if saved:
...
## Validação em equipamento real — feita, com 2 bugs reais encontrados e corrigidos

Depois das 10 tasks prontas e revisadas, o fluxo completo (excluir →
autorizar de novo → reiniciar) foi testado contra a ONU real `0/4/6` na OLT
**BARRA DE SAO MIGUEL** (100.65.10.200, tenant `rads`, `olt_id=7` —
trocada da SANTANA originalmente planejada porque o usuário conhecia com
certeza quais câmeras essa posição específica atende nessa OLT, MAC
`98:2a:0a:9b:b0:12`, VLAN 3000). Rodado direto contra produção: o driver
novo (`app/cli/tools/olt_4840e_add_onu.py`) foi copiado pro container
`sightops-prod-api` via `docker cp` (sem trocar a imagem), e chamado
diretamente (sem passar por `olt_service.py`, que ainda não tinha esse
deploy) via `docker exec ... python3 -c "..."`, no mesmo padrão já usado
nesta sessão pra exploração segura da 4840E.

**Excluir**: `delete_onu_4840e` reportou `ok=True`. Confirmado via
`show white-list` que o MAC realmente saiu da whitelist. Achado real (não é
bug, é comportamento da OLT): tirar da whitelist **não** derruba o link
físico na hora — a ONU continua `Up`/passando tráfego até precisar
renegociar. Isso é diferente do que se esperaria de uma GPON — vale saber
na hora de explicar pro cliente ou pro técnico.

**Autorizar de novo**: encontrou 2 bugs reais que nenhum teste com mock
pegou (porque mock nunca valida a sintaxe real do comando contra o
equipamento):

1. **`interface ethernet {porta}` estava sem o prefixo de slot** — o
   comando certo é `interface ethernet 0/{porta}` (mesmo padrão de
   `interface pon 0/{pon}`). Sem o prefixo, a OLT respondia `% Invalid
   parameter`, e a ONU ficava autorizada na whitelist mas **sem VLAN
   nenhuma aplicada** — o mesmo tipo de estado parcial que motivou o
   `OnuAddError` carregar `onu=` pra permitir recuperação. Corrigido em
   `add_onu_4840e` (Task 4, acima).
2. **`copy running-config startup-config` também pede confirmação y/n**
   nesta OLT (`"Startup config in flash will be updated, are you sure
   (y/n)? [n]"`), igual ao `onu-reboot`. Sem responder, a config nunca é
   salva na flash — mas como o texto de confirmação não bate com nenhum
   `_FAILURE_MARKERS`, o driver reportava `saved: True` mesmo sem ter
   salvo nada de verdade. **Isso significa que a exclusão de teste também
   não tinha sido salva de verdade até esse ponto** — foi corrigido e
   re-salvo manualmente antes de prosseguir, confirmado com
   `"Update startup config successfully."` na saída real da OLT.

Correção adotada (aplicada em `add_onu_4840e` e `delete_onu_4840e`,
Tasks 4 e 5 acima): um novo helper `_cli_confirm_save`, mesmo padrão de
`_cli_confirm_reboot` (espera o texto de confirmação, responde `y` de
forma controlada, nunca conta com o próximo comando da fila):
...
(No código real, `_CONFIRM_YN_RE`/`_GENERIC_PROMPT_RE`/`_cli_confirm_save`
ficam perto do topo do arquivo — logo depois de `_validate_mac_shape`, Task
1 — e `_cli_confirm_reboot`, definida bem mais abaixo perto de
`reboot_onu_4840e`, Task 6, reaproveita as mesmas duas regexes sem
redefini-las.)

`scripts/sightops_olt_4840e_add_onu_test.py` ganhou um `_SAVE_CONFIRM_STEPS`
(dict reutilizável simulando a confirmação real) mesclado nos 4 testes que
exercitam o caminho de salvar com sucesso, mais uma checagem explícita de
que `"y"` aparece em `commands_run` nesses casos — mesmo padrão de rigor já
usado nos testes do `reboot_onu_4840e`.

**Reiniciar**: `reboot_onu_4840e` reportou `ok=True`, `y` confirmado.
Verificado via `onu_signal_4840e`: a ONU caiu (`state: Down`,
`register_time` vazio) e voltou sozinha ~45s depois com `register_time`
novo (data/hora do teste) e sinal ótico normal (RX -24.6 dBm, igual a
antes) — prova de reinício real, não só de comando aceito.

Depois da validação (com os 2 bugs já corrigidos e re-testados
manualmente contra a mesma OLT), o deploy real em produção segue o
processo já usado nesta sessão pra 8820i: extrair o `olt_service.py` real
do container (`docker cp`), gerar o diff a partir do último commit
deployado, aplicar com `patch --fuzz=3`, validar com
`py_compile`/`import app.main` dentro da imagem, e trocar via
`deploy_api.py`/`deploy_api_v3.py`. Isso **não** é uma task deste plano —
acontece quando o usuário pedir o deploy.
